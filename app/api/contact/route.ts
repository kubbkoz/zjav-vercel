...
export async function POST(req: NextRequest) {
  ...
  const { name, email, phone, message, honeypot, formLoadedAt } = body;
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      to: ADMIN_EMAIL,
      subject: `Nový dopyt od ${name} — zjav.sk`,
      html: `
        <div style="font-family: monospace; background:#0a0f1a; color:#f0f4ff; padding:32px; border-radius:8px; max-width:600px;">
          ...
          <table style="width:100%; border-collapse:collapse;">
            <tr><td style="color:#a0abcb; padding:8px 0; width:120px;">Meno:</td><td style="color:#f0f4ff;">${name}</td></tr>
            <tr><td style="color:#a0abcb; padding:8px 0;">Email:</td><td style="color:#00cfe8;"><a href="mailto:${email}" style="color:#00cfe8;">${email}</a></td></tr>
            ${phone ? `<tr><td style="color:#a0abcb; padding:8px 0;">Telefón:</td><td style="color:#f0f4ff;">${phone}</td></tr>` : ""}
            <tr><td style="color:#a0abcb; padding:8px 0; vertical-align:top;">Správa:</td><td style="color:#f0f4ff; white-space:pre-line;">${message}</td></tr>
          </table>
    ...
    await transporter.sendMail({
      ...
      to: email,
      subject: `Dostal som váš dopyt, ${name.split(" ")[0]}!`,
      html: `
        <div style="font-family: monospace; background:#0a0f1a; color:#f0f4ff; padding:32px; border-radius:8px; max-width:600px;">
          <div style="border-bottom:1px solid #1e3a5f; padding-bottom:16px; margin-bottom:24px;">
            <span style="font-size:24px; font-weight:700; color:#00cfe8;">ZJAV_</span>
          </div>
          <p style="color:#f0f4ff; font-size:16px; line-height:1.7;">Ahoj <strong>${name.split(" ")[0]}</strong>,</p>
          ...
          <div style="margin-top:32px; padding:16px; border:1px solid #1e3a5f; border-radius:4px;">
            ...
            <p style="color:#f0f4ff; white-space:pre-line; margin:0;">${message}</p>
          </div>
